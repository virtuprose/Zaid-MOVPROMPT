...
export function AuthGateDialog({ open, onOpenChange, returnPath }: {
  ...
  const startOAuth = async (nextProvider: "google" | "apple") => {
    ...
    const result = await lovable.auth.signInWithOAuth(nextProvider, { redirect_uri: redirectUri });
    if (result.error) {
      setProvider(null);
      toast.error(`Could not continue with ${nextProvider === "google" ? "Google" : "Apple"}. Your campaign is still saved in this browser.`);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="creator-auth-gate sm:max-w-[440px]">
        ...
        <div className="creator-auth-gate-actions">
          ...
          <button className="creator-button creator-button-quiet" type="button" onClick={() => navigate(`/auth?next=${encodeURIComponent(returnPath)}`)} disabled={provider !== null}>
            Continue with email
          </button>
        </div>
