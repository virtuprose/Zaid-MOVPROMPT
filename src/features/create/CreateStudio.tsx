...
import { estimateGenerationQuote, projectToCreationDraft, type CreationDraft, type GenerationQuote } from "./contracts";
...
export function CreateStudio({ qaMode = false }: { qaMode?: boolean }) {
  ...
  const template = getCreatorTemplate(project.templateId);
  const fallbackQuote = useMemo(() => estimateGenerationQuote(template.duration, !project.videoUrl), [project.videoUrl, template.duration]);
  ...
  useEffect(() => {
    ...
    void supabase.functions.invoke("generation-quote", { body: { capability: "video.seedance.latest", duration_seconds: template.duration, template_id: template.id } }).then(({ data, error }) => {
      ...
      else setQuote(fallbackQuote);
      setQuoteLoaded(true);
    });
    return () => { active = false; };
  }, [fallbackQuote, template.duration, template.id, user?.id]);
  ...
  useEffect(() => {
    if (qaMode || !user || !shouldResumeGeneration || draftRestoring || !quoteLoaded || resumedGeneration.current) return;
  ...
  }, [draftRestoring, project.pendingGenerationId, project.pendingQuoteCredits, project.product.images.length, qaMode, quote.credits, quoteLoaded, rightsConfirmed, shouldResumeGeneration, step, user]);
        ...
        {step === "details" && (
          <div className="creator-workspace">
            <section className="creator-panel creator-panel-pad" aria-labelledby="campaign-heading">
              ...
              <div className="creator-actions-row"><button className="creator-button creator-button-quiet" type="button" onClick={() => setStep(templateFirst.current ? "source" : "template")}><ArrowLeft aria-hidden="true" /> Back</button><button className="creator-button creator-button-primary" type="button" onClick={() => void startGeneration()} disabled={!project.product.name.trim() || !rightsConfirmed || !quoteLoaded || sourceBusy}><Sparkles aria-hidden="true" /> Generate video</button></div>
            </section>
            ...
            <aside className="creator-panel creator-panel-pad" aria-label="Generation summary">
              ...
              <div className="creator-cost-box"><small>{quote.entitlementEligible ? "Starter render eligible" : "Estimated generation"}</small><strong>{quote.entitlementEligible ? `Included for eligible new accounts · otherwise ${quote.credits} credits` : `${quote.credits} credits`} · usually 2–5 minutes</strong></div>
            </aside>
          </div>
        )}
      </div>}
