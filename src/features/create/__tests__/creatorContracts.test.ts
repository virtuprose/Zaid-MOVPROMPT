...
import { estimateGenerationQuote, projectToCreationDraft } from "../contracts";
...
describe("creator contracts", () => {
  ...
  it("uses the approved Seedance capability and duration-based quote", () => {
    const quote = estimateGenerationQuote(8, true);
    expect(quote.capability).toBe("video.seedance.latest");
    expect(quote.credits).toBe(144);
  ...
  it("preserves the pending generation intent in a seven-day guest draft", () => {
    ...
    expect(draft.acceptedQuote?.credits).toBe(180);
