...
assert_query "restricted API role can manage owner-scoped guest claims" "SELECT (has_table_privilege('movprompt_api', 'guest_claim_operations', 'SELECT,INSERT,UPDATE,DELETE') AND has_table_privilege('movprompt_api', 'guest_claim_assets', 'SELECT,INSERT,UPDATE,DELETE'))::int;"
assert_query "restricted worker role can reconcile abandoned guest claims" "SELECT (has_table_privilege('movprompt_worker', 'guest_claim_operations', 'SELECT,UPDATE') AND has_table_privilege('movprompt_worker', 'guest_claim_assets', 'SELECT,UPDATE'))::int;"
assert_query "guest claim privileges remain private" "SELECT (NOT has_table_privilege('public', 'guest_claim_operations', 'SELECT') AND NOT has_table_privilege('public', 'guest_claim_assets', 'SELECT'))::int;"
