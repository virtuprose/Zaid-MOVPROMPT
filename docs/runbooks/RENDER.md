...
| `SMTP_HOST` | Production SMTP provider |
| `SMTP_PORT` | SMTP provider port |
| `SMTP_SECURE` | `true` for implicit TLS, otherwise `false` for STARTTLS |
| `SMTP_USER` | SMTP credential, if required |
| `SMTP_PASSWORD` | SMTP credential, if required |
| `EMAIL_FROM` | Verified sender, such as `MovPrompt <no-reply@domain>` |
