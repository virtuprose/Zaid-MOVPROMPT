...
- DO NOT use this tool to make art the user didn't ask for. DO NOT use it as a substitute for video. Avoid regenerating the same character on a whim. HOWEVER, generate additional \`character_sheet\` calls when the user asks for a NEW or DIFFERENT character (co-star, second protagonist, antagonist, sidekick). When doing so, DO NOT pass the existing character's image in \`reference_urls\` — only attach a reference if the user uploaded a new photo for that new character. The new sheet must establish a fresh identity, not re-pose the previous one.
