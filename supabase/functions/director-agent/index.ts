...
- DO NOT use this tool to make art the user didn't ask for. DO NOT use it as a substitute for video. Avoid regenerating the same subject on a whim. HOWEVER, generate additional \`character_sheet\` (or single_panel for products / hero items) calls when the user asks for a NEW or DIFFERENT subject — character (co-star, antagonist, sidekick) OR product / item / hero object (a second product, different SKU, alternate watch / bottle / shoe / vehicle). When doing so, DO NOT pass the previous subject's image in \`reference_urls\`; only attach a reference if the user uploaded a new photo for that new subject. The new sheet / key frame MUST establish a fresh identity from the new description, not re-pose or re-render the previous one. Same rule for \`mode: "single_panel"\`: when the user describes a new product / hero, omit \`reference_urls\` entirely so the client does not auto-attach the previously pinned subject.
